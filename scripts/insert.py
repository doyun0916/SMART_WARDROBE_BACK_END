...
from coordi.serializers import WtravelSerializer  ## 각각 style별로 바꿔주면됨 넣을때 마다

import json

with open("./mss_woman_travel.json", "r") as json_file:
        json_data = json.load(json_file)

num = len(json_data)
def run():
    for i in range(num):
        insert_check = WtravelSerializer(data=json_data[i])    ### 이 친구도 마찬가지
