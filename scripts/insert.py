...
from coordi.serializers import WworkwearSerializer  ## 각각 style별로 바꿔주면됨 넣을때 마다

import json

with open("./street_snap_woman_workwear.json", "r") as json_file:
        json_data = json.load(json_file)

num = len(json_data)
def run():
    for i in range(num):
        insert_check = WworkwearSerializer(data=json_data[i])    ### 이 친구도 마찬가지
