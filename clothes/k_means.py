...
def image_color_cluster_jean(image_path, json_path, k=3):  # k값은 몇개의 색깔 인지
    image = cv2.imread(image_path)
    image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)

    with open(json_path) as f:
        json_data = json.load(f)
        json_mask = np.array(json_data["result"]["objects"][0]["pred_masks"])  # 상림 json형식
    n = image[json_mask]
    ...
    hist = np.sort(hist)[::-1]  # 색깔비율 내림차순 정렬
    for i in range(k):
        print(find_color_jean(clt.cluster_centers_[i]))
